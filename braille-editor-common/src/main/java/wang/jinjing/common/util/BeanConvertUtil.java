...
public class BeanConvertUtil {
    ...
    public static <E extends BaseEntity, V extends BaseVO> Page<V> convertToVoPage(Class<V> voClass, Page<E> tPage) {
        List<E> records = tPage.getRecords();
        List<V> voList = convertToVoList(voClass, records);
        Page<V> voPage = new Page<>();
        voPage.setRecords(voList);
        voPage.setTotal(tPage.getTotal());
        voPage.setCurrent(tPage.getCurrent());
        voPage.setSize(tPage.getSize());
        return voPage;
    }
